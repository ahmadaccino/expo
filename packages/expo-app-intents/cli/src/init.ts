...
async function assertDirectoryIsNotAlreadyConfiguredAsync(
  ...
  if (!pluginEntry) {
    // A dynamic config can add the plugin after the static config is read, so "no entry" here does
    // not mean the project has none. Inspect the evaluated watched trees instead: an existing setup
    // anywhere other than the requested path is concrete evidence that this run would create a
    // duplicate, while a tree with no other setup is safe to scaffold.
