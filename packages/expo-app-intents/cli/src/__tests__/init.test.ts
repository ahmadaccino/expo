...
describe(runInit, () => {
  ...
  it('reports nothing about the app config when the config already has everything', async () => {
    writeProject(
      staticConfig({
        plugins: ['expo-app-intents'],
        experiments: { inlineModules: { watchedDirectories: ['app-intents'] } },
    ...
    const logged = messages(log);
    expect(logged).not.toContain('Enabled inline modules');
    expect(logged).not.toContain("Added 'expo-app-intents'");
    expect(logged).toContain('Created in app-intents/');
  });

  it('says the plugin was repointed rather than added', async () => {
    writeProject(
      staticConfig({
        plugins: [['expo-app-intents', { directory: 'siri' }]],
    ...
    const logged = messages(log);
    expect(logged).toContain("Pointed 'expo-app-intents' at 'app-intents'");
    expect(logged).not.toContain("Added 'expo-app-intents'");
  });
  ...
  it('names both directories and the way out when it refuses to repoint', async () => {
    writeProject(
      staticConfig({
        plugins: [['expo-app-intents', { directory: 'siri' }]],
        experiments: { inlineModules: { watchedDirectories: ['siri'] } },
    ...
    const message = (error as Error).message;
    expect(message).toContain('siri');
    expect(message).toContain('app-intents');
    expect(message).toContain('--dir siri');
    expect(message).toContain('watchedDirectories');
  });
...
`);
  ...
  // Nothing is watched yet, so no other directory is compiled into the app target and an unseen
  // plugin entry has nowhere to collide. A first-time setup must not be warned about it.
  it('does not warn about the plugin check when a dynamic config watches nothing', async () => {
    writeProject(null);
    write({
      'project/app.config.js': `module.exports = { expo: { name: 'my-app', slug: 'my-app' } };`,
    });
    ...
    expect(messages(warn)).not.toContain('plugins list is computed in JavaScript');
  });

  // The only watched directory is the one this run scaffolds, so there is still no second directory
  // an unseen plugin entry could point at.
  it('does not warn about the plugin check when a dynamic config watches only the requested directory', async () => {
    writeProject(null);
    write({
